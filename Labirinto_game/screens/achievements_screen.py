...
def carregar_e_redimensionar_icone(caminho, tamanho=(80, 80)):
...
def tela_conquistas(tela, usuario):
    ...
    while True:
        ...
        for chave, conquista in conquistas.items():
            ...
            icone_rect = pygame.Rect(titulo_x, y_offset, 80, 80)
            ...
            if chave in icones and icones[chave]:
                ...
                if not conquista['desbloqueada']:
                    ...
                    cinza_surface = pygame.Surface((80, 80), pygame.SRCALPHA)
                ...
                pygame.draw.rect(tela, cor_com_escala_cinza(200, 200, 200), icone_rect, width=2, border_radius=10)
            else:
                # Fallback para o retângulo original caso o ícone não seja encontrado
                pygame.draw.rect(tela, cor_com_escala_cinza(150, 150, 150), icone_rect, border_radius=10)
                pygame.draw.rect(tela, cor_com_escala_cinza(200, 200, 200), icone_rect, width=2, border_radius=10)
            
            # Desenha informações da conquista
            desenhar_texto(conquista['nome'], fonte_texto, COR_TEXTO, tela, titulo_x + 100, y_offset)
            desenhar_texto(conquista['descricao'], pygame.font.SysFont("comicsansms", 30), 
                          cor_com_escala_cinza(200, 200, 200), tela, titulo_x + 100, y_offset + 40)
            desenhar_texto(status, pygame.font.SysFont("comicsansms", 25), 
                          cor_status, tela, LARGURA_TELA - 300, y_offset + 20)
